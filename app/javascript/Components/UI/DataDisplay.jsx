  ...
  if (src) {
    return (
      <img
        src={src}
        alt={name}
        className={`avatar ${sizes[size]} object-cover ${className}`}
      />
    )
  }
...
export const AvatarGroup = ({ avatars = [], max = 4, size = 'md' }) => {
  ...
  return (
    <div className="flex -space-x-2">
      ...
      {remaining > 0 && (
        <div className={`avatar ${size === 'md' ? 'avatar-md' : 'avatar-sm'} ring-2 ring-white bg-gray-200 text-gray-600`}>
          +{remaining}
        </div>
      )}
    </div>
  ...
  return (
    <div className={className}>
      <div className={`progress ${sizes[size]}`}>
        <div 
          className={`progress-bar bg-gradient-to-r ${colors[color]}`}
  ...
  <div className={`relative ${className}`}>
    ...
    {label && (
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="bg-gray-50 px-3 text-sm text-gray-500">{label}</span>
      </div>
    )}
  </div>
)
...
export const Tooltip = ({ children, content, position = 'top' }) => {
  ...
  return (
    ...
    >
      {children}
      {show && content && (
        <div className={`absolute z-50 px-2 py-1 text-xs text-white bg-gray-900 rounded-lg whitespace-nowrap ${positions[position]}`}>
          {content}
        </div>
      )}
    </div>
